import { signReduser as reducer, initialState } from "./sign";
import {
    TSignActions
} from "../actions";
...
describe("audit signReduser", () => {
    it("should return the initial state", () => {
        expect(
            reducer(undefined, {} as TSignActions)
        ).toEqual(
            initialState
        );
    });
    ...
    it(`should handle get auth request success - ${actionsTypes.GET_AUTH_SUCCESS}`, () => {
      const payload = mocks.dataTSignData as types.TSignData;
        expect(
            reducer(
                initialState,
                actions.getAuthSuccess(payload)
            )
        )
        .toEqual(
            {
    ...
    it(`should handle get register request success - ${actionsTypes.GET_REGISTER_SUCCESS}`, () => {
        const payload = mocks.dataTSignData as types.TSignData;
          expect(
              reducer(
                  initialState,
                  actions.getRegisterSuccess(payload)
              )
          )
          .toEqual(
              {
      ...
      it(`should handle get profile request success - ${actionsTypes.GET_PROFILE_SUCCESS}`, () => {
        const payload = mocks.dataTSignData as types.TSignData;
          expect(
              reducer(
                  initialState,
                  actions.getProfileSuccess(payload)
              )
          )
          .toEqual(
              {
